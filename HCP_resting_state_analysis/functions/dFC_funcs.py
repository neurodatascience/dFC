...
def TR_intersection(dFCM_lst):  # input is a list of dFCM objs
    ...
    for dFCM in dFCM_lst:
        assert dFCM.TS_info["Fs"] == common_Fs, "Fs mismatch. Cannot find the common TRs"
...
class SIMILARITY_ASSESSMENT:
    ...
    def assess_similarity(self, dFCM_lst, downsampling_method="default", **param_dict):
        ...
        FO_lst = self.FO_calc(dFCM_lst, common_TRs=common_TRs)
        methods_assess["FO"] = FO_lst

        ########## transition frequency ##########

        transition_stats_lst = self.transition_stats(dFCM_lst, common_TRs=common_TRs)
        ...
        if parallelize:
            out_lst = Parallel(n_jobs=4, verbose=0, backend="loky")(
                delayed(self.assess_similarity)(
                    dFCM_lst=filter_dFCM_lst(self.dFCM_lst, **FILTERS[filter]),
        ...
        for subject in SUBJECTs:
            ...
            if SW_dFC is None:
                ...
                SW_dFC = np.concatenate(
                    (SW_dFC, dFCM_raw.get_dFC_mat(TRs=dFCM_raw.TR_array)), axis=0
                )
...
class HMM_DISC(dFC):
    ...
    def estimate_FCS(self, time_series):
        ...
        Obs_seq = None
        for subject in SUBJECTs:
            new_dFCM = self.swc.estimate_dFCM(
                time_series=time_series.get_subj_ts(subjs_id=subject)
            )
...
class DFCM:
    ...
    def set_dFC(self, FCSs, FCS_idx=None, TS_info=None, TR_array=None):
        ...
        assert self.n_time == -1, "why n_time is not -1 ?"
...
class DATA_LOADER:
    ...
    def load_gordon(self, subj_id2load=None):
        ...
        SESSIONs = self.params[
            "SESSIONs"
        ]  # ['Rest1_LR' , 'Rest1_RL', 'Rest2_LR', 'Rest2_RL']
        ...
        nodes2include = [
            i - 1
            for i, x in enumerate(atlas_data)
            if x[3] in self.params["networks2include"]
        ]
        ...
        for session in SESSIONs:
            ...
            for subject in SUBJECTS:
                ...
                DATA = hdf5storage.loadmat(
                    self.params["data_root_gordon"]
                    + subj_fldr
                    + "/ROI_data_Gordon_333_surf.mat"
                )
