...
class Test_TIME_SERIES_CLASS:
    ...
    def test_get_subj_ts(self):
        ...
        ########## normal append_ts ##############
        time_series = TIME_SERIES(data=self.data, subj_id='1', Fs=2, time_array=self.time_array2)
        time_series.append_ts(new_time_series=self.data2, subj_id='2', time_array=self.time_array2)

        assert time_series.n_time_==2400, \
            "n_time_ property failed."
        assert time_series.n_time==2400, \
            "n_time property failed."
        assert time_series.n_regions_==333, \
            "n_regions_ property failed."
        assert time_series.n_regions==333, \
            "n_regions property failed."

        assert time_series.interval_==list(range(time_series.n_time_)), \
            "interval property failed."
        assert time_series.interval.tolist()[0]==list(range(time_series.n_time_)), \
            "interval property failed."
        
        assert np.all(time_series.get_subj_ts('1').data_==self.data), \
            "get_subj_ts method failed."
        assert np.all(time_series.get_subj_ts('1').data==self.data), \
            "get_subj_ts method failed."
        assert np.all(time_series.get_subj_ts('2').data_==self.data2), \
            "get_subj_ts method failed."
        assert np.all(time_series.get_subj_ts('2').data==self.data2), \
            "get_subj_ts method failed."

        assert np.unique(time_series.get_subj_ts('1').subj_id_array)=='1', \
            "get_subj_ts method failed."
        assert np.unique(time_series.get_subj_ts('2').subj_id_array)=='2', \
            "get_subj_ts method failed."
        ...
        assert time_series.n_regions_==333, \
            "n_regions_ property failed."
        assert time_series.n_regions==50, \
            "n_regions property failed."

        assert np.all(time_series.get_subj_ts('1').data_==self.data[nodes_idx,:]), \
            "get_subj_ts method failed."
        assert np.all(time_series.get_subj_ts('1').data==self.data[nodes_idx,:]), \
            "get_subj_ts method failed."
        assert np.all(time_series.get_subj_ts('1').get_subj_ts('1').get_subj_ts('1').data==self.data[nodes_idx,:]), \
            "get_subj_ts method failed."
        assert np.all(time_series.get_subj_ts('2').data_==self.data2[nodes_idx,:]), \
            "get_subj_ts method failed."
        assert np.all(time_series.get_subj_ts('2').data==self.data2[nodes_idx,:]), \
            "get_subj_ts method failed."
        assert np.all(time_series.get_subj_ts('2').get_subj_ts('2').get_subj_ts('2').data==self.data2[nodes_idx,:]), \
            "get_subj_ts method failed."
        ...
        assert np.all(time_series.get_subj_ts('1').data==self.data), \
            "get_subj_ts method failed."
        assert np.all(time_series.get_subj_ts('1').get_subj_ts('1').get_subj_ts('1').data==self.data), \
            "get_subj_ts method failed."
        assert np.all(time_series.get_subj_ts('2').data_==self.data2), \
            "get_subj_ts method failed."
        assert np.all(time_series.get_subj_ts('2').data==self.data2), \
            "get_subj_ts method failed."
        assert np.all(time_series.get_subj_ts('2').get_subj_ts('2').get_subj_ts('2').data==self.data2), \
            "get_subj_ts method failed."

    def test_n_time(self):
        time_series = TIME_SERIES(data=self.data, subj_id='1', Fs=2)
        assert time_series.n_time_==1200, \
            "n_time property failed."
        assert time_series.n_time==1200, \
            "n_time property failed."
        time_series.truncate(start_point=10, end_point=100)
        assert time_series.n_time_==1200, \
            "TRUNCATE: n_time property failed."
        assert time_series.n_time==91, \
            "TRUNCATE: n_time property failed."
        time_series.truncate()
        assert time_series.n_time_==1200, \
            "TRUNCATE_RESET: n_time property failed."
        assert time_series.n_time==1200, \
            "TRUNCATE_RESET: n_time property failed."
    ...
    def test_node_lst_after_append(self):
        ...
        assert np.all(time_series.nodes_lst==np.array(nodes_idx)[:, np.newaxis]), \
            "keeping node selection after append_ts failed."
